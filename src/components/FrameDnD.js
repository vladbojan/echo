...
export default function FrameDnD(props)  {
      ...
      >
        {(createDraft, { data, loading, error }) => {
          return (
              ...
              >
                <div ref={drop} className={isActive ? classes.dndHover:classes.dnd}>{isActive ? 'Muta cadrul aici' : '   '}</div>
...
const EDIT_DRAFT_MUTATION = gql`
  mutation MoveFrameMutation($id: ID!, $position: String!) {
    updateFramePosition(id: $id, position: $position) {
      id
      position
    }
  }
